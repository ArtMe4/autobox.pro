    ...
    if ( $the_query->have_posts() ) {
      ...
      while( $the_query->have_posts() ) : $the_query->the_post();
        ...
        if ( empty($title) || $title == '' ) {
          $carousel_items .= '<div class="like-h6"><a href="' . esc_url( get_permalink(get_the_ID()) ) . '" rel="bookmark">' . esc_html__( 'Click here to read more', 'tz-feature-pack' ) . '</a></div>';
        } else {
          $carousel_items .= '<div class="like-h6"><a href="' . esc_url( get_permalink(get_the_ID()) ) . '" rel="bookmark">' . esc_attr($title) . '</a></div>';
        }
				if ( 'style-2' != $settings['widget_style'] ) {
        	$carousel_items .= '<div class="time-wrapper">' . get_the_date() . '</div>';
				}
