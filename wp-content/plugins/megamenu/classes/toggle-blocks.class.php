...
class Mega_Menu_Toggle_Blocks {
    ...
    public function output_menu_public_toggle_block_html( $html, $settings ) {
        ...
        if ( ! $css_version || version_compare($css_version, '2.4.0.2') >= 0 ) {
            $closed_text = isset( $settings['closed_text'] ) ? do_shortcode( stripslashes( $settings['closed_text'] ) ) : "Меню";
            $open_text = isset( $settings['open_text'] ) ? do_shortcode( stripslashes( $settings['open_text'] ) ) : "Меню";
    ...
    private function get_default_menu_toggle_block( $theme_id = 'default' ) {
        ...
        $defaults = array(
            'type' => 'menu_toggle',
            'align' => 'right',
            'closed_text' => isset($menu_theme['responsive_text']) ? $menu_theme['responsive_text'] : "Меню",
            'open_text' => isset($menu_theme['responsive_text']) ? $menu_theme['responsive_text'] : "Меню",
