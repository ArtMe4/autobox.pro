...
if ( ! function_exists( 'chromium_chromify_title' ) ) :
 ...
 if ( ! function_exists( 'chromium_entry_author' ) ) {
   function chromium_entry_author() {
       printf( '<div class="post-author" itemprop="author" itemscope="itemscope" itemtype="https://schema.org/Person">'.esc_html__(' by ', 'chromium').'<a href="%1$s" title="%2$s" rel="author" itemprop="url"><span itemprop="name">%3$s</span></a></div>',
      ...
      if ( is_array($logo) ) {
        echo '<span itemprop="logo" itemscope="itemscope" itemtype="https://schema.org/ImageObject"><meta itemprop="url" content="'.esc_url($logo[0]).'"></span>';
      }
       echo '</span>';
   }
 }
  	...
  	class chromium_comments_walker extends Walker_Comment {
  	    ...
  	    function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
  	      ...
  	      $add_below = 'comment';
  				...
  				<?php } else { ?>
      	    <article <?php comment_class(empty( $args['has_children'] ) ? '' :'parent') ?> id="comment-<?php comment_ID() ?>" itemprop="comment" itemscope="itemscope" itemtype="https://schema.org/UserComments">
      	      <figure class="gravatar"><?php echo get_avatar( $comment, 60, '', esc_html__("Author's gravatar", 'chromium') ); ?></figure>

      	      <header class="comment-meta">
      	        <h2 class="comment-author" itemprop="creator" itemscope="itemscope" itemtype="https://schema.org/Person">
