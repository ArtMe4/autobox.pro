<?php
/**
 * Template part for displaying quote post format posts
 */
?>

<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemscope="itemscope" itemtype="https://schema.org/Article"><!-- Article ID-<?php the_ID(); ?> -->

	<?php /* Get Featured Video if exist */
	$featured_quote = get_post_meta( get_the_ID(), 'chromium_featured_quote', true);
	if ( $featured_quote && $featured_quote!='' ) {
		$featured_quote_size = 'post-thumbnail';
		if ( get_theme_mod( 'grid_blog', false ) ) {
			$featured_quote_size = 'chromium-grid-blog';
		}
		echo '<div class="thumbnail-wrapper quote">';
		if ( has_post_thumbnail() && ! post_password_required() ) { the_post_thumbnail( $featured_quote_size ); }
		...
		<div class="thumbnail-wrapper" itemprop="image" itemscope="itemscope" itemtype="https://schema.org/ImageObject"><!-- Article's Featured Image -->
